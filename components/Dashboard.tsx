...
export const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const recentActivity = [
    ...
    {
      type: 'view',
      item: 'Baghdad International Music Festival',
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      {statusMsg && (
        <div
          className={`fixed bottom-8 right-8 px-6 py-3 rounded-xl backdrop-blur-xl border shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-4 ${
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-12 text-center md:text-start rtl:md:text-right">
        ...
        <button
          onClick={onLogout}
          className="px-6 py-2 rounded-full bg-accent/20 border border-accent text-accent font-semibold hover:bg-accent/30 transition-colors"
        >
          {t('dashboard.logout')}
        </button>
      </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="flex p-1 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10">
            <button
              ...
              className={`px-8 py-2.5 rounded-xl transition-all font-medium ${
                activeTab === 'profile' ? 'bg-primary text-white shadow-glow-primary' : 'text-white/60 hover:text-white'
              }`}
            >
              My Profile
            </button>
            <button
              ...
              className={`px-8 py-2.5 rounded-xl transition-all font-medium ${
                activeTab === 'architect' ? 'bg-primary text-white shadow-glow-primary' : 'text-white/60 hover:text-white'
              }`}
            >
              Data Architect
            </button>
            ...
            <div className="max-w-2xl mx-auto mb-12">
              ...
              <GlassCard className="p-6 border-primary/30 bg-primary/10">
                <p className="text-white/80 text-sm">
                  <span className="font-semibold text-primary">MVP boundary:</span> Business posting tools are disabled until
                  moderation and storage workflows are completed in Phase 2.
                </p>
              </GlassCard>
            </div>
          )}
              ...
              <form className="space-y-4" onSubmit={handleProfileUpdate}>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white outline-none focus:border-primary transition-colors"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    type="password"
                    placeholder="********"
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white/40 outline-none cursor-not-allowed"
                    disabled
                  />
                ...
                <button
                  type="submit"
                  disabled
                  className="w-full !mt-6 py-3 rounded-xl bg-gradient-to-r from-primary to-secondary text-white font-semibold transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                >
            ...
            <div className="lg:col-span-2 space-y-8">
              <GlassCard className="p-6">
                <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-3">
                  <Heart className="text-accent" /> {t('dashboard.myFavorites')}
                </h2>
                <div className="space-y-4 max-h-60 overflow-y-auto pr-2 rtl:pr-0 rtl:pl-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-white/60">
                        <div className="flex items-center gap-1">
                          <Star className="w-4 h-4 text-accent" /> 4.5
                        </div>
                        <div className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-4 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                    <img
                      src="https://picsum.photos/seed/b1/128/128"
                      alt="Grand Millennium"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-white/60">
                        <div className="flex items-center gap-1">
                          <Star className="w-4 h-4 text-accent" /> 4.8
                        </div>
                        <div className="flex items-center gap-1">
                          <MapPin className="w-4 h-4" /> 2.5 km
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div key={index} className="flex items-center gap-4 p-3 rounded-xl bg-white/5">
                      ...
                      <div className="flex-1">
                        <p className="text-white/90 text-sm">
                          <span className="font-semibold">{t(`activity.${activity.type}`)}:</span> {activity.item}
                        </p>
                      </div>
